...
class Loss_CategoricalCrossentropy(Loss):
    def forward(self, y_pred, y_true):
        ...
        if len (y_true.shape) == 1:
            correct_confidencies = y_pred_clipped[range(samples), y_true]
        elif len(y_true.shape) == 2:
            correct_confidencies = np.sum(y_pred_clipped*y_true, axis=1)
        negative_log_likelihoods = -np.log(correct_confidencies)
